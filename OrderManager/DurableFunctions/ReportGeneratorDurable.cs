...
{
    public static class ReportGeneratorDurable
    {
        [FunctionName("ReportGeneratorDurable")]
        [StorageAccount("StorageAccount")]
        public static async Task Run([TimerTrigger("%RepoterScheduleTriggerTime%")] TimerInfo myTimer,
            [Table("ordersTableDurable", "Order")] CloudTable table,
            IBinder reportBinder,
            ILogger log)
        {
            log.LogInformation($"ReportGeneratorDurable executed at: {DateTime.Now}");
            ...
            if (ordersToReport.Any())
            {
                ...
                using (var reportBlob = reportBinder.Bind<TextWriter>(new BlobAttribute($"reportsdurable/{reportDate:yyyyMMddHHmmss}.rep")))
                {
